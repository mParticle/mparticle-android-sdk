...
class ReportingServiceTest : BaseMPServiceTest() {
    ...
    fun testEntryIntegrity() {
        ...
        Collections.sort(
            ...
            java.util.Comparator { o1, o2 ->
                try {
                    return@Comparator o1.toJson().getInt("a random Number").compareTo(
    ...
    private fun getNReportingMessages(n: Int, sessionId: String?): List<JsonReportingMessage> {
        ...
        for (i in 0 until n) {
            reportingMessages.add(
                ReportingTableTest.getRandomReportingMessage(sessionId ?: Random.Default.nextInt().toString())
            )
        }
    ...
    private fun equals(
        ...
        return reportingMessage.sessionId == jsonReportingMessage.sessionId && reportingString == origStirng
    }
}
