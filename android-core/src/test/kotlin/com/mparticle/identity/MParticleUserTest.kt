...
class MParticleUserTest {
    ...
    fun testAddExistingUserIdentity() {
        ...
        Mockito.`when`(
            id?.mMessageManager?.logUserIdentityChangeMessage(
                ...
                ), Mockito.any(JSONObject::class.java), Mockito.any(
    ...
    fun testChangeUserIdentity() {
        ...
        Mockito.verify(MParticle.getInstance()?.Internal()?.kitManager, Mockito.times(1))
            ?.setUserIdentity(
                ArgumentMatchers.eq("email value 2"), ArgumentMatchers.eq(
                    IdentityType.Email
                )
            )
    ...
    fun testRemoveUserTag() {
        ...
        Mockito.verify(mp?.Internal()?.kitManager, Mockito.times(1))?.removeUserAttribute("test", 1)
    }
    ...
    fun testGetAllUserAttributes() {
        ...
        Mockito.verify(mp?.Identity()?.mMessageManager, Mockito.times(1))?.getUserAttributes(
        ...
        Mockito.verify(mp?.Identity()?.mMessageManager, Mockito.times(2))?.getUserAttributes(
            ArgumentMatchers.any(
                UserAttributeListenerWrapper::class.java
            ), ArgumentMatchers.eq<Long>(defaultMpId)
        )
    ...
    fun testRemoveUserIdentityWhenNoneExist() {
        ...
        Mockito.verify(mp?.Internal()?.kitManager, Mockito.times(0))?.removeUserIdentity(
            Mockito.any(
    ...
    fun testAddUserIdentity() {
        ...
        Assert.assertEquals(oldIdentity["n"], IdentityType.Alias.value)
        Assert.assertEquals(
        ...
        Mockito.verify(mp?.Internal()?.kitManager, Mockito.times(1))?.setUserIdentity(
            ArgumentMatchers.eq("alias test"), ArgumentMatchers.eq(
                IdentityType.Alias
            )
        )
        val allIdentities = argument3.value
