...
public class AbstractKitTest {
    ...
    public void testParseConfig() throws Exception {
        AbstractKit provider = new FakeProvider()
            ...
            if (ekConfig.has("avf")) {
                ...
                if (attributeValueFilter.has("i") && attributeValueFilter.has("a") && attributeValueFilter.has("v")) {
                    ...
                    if (shouldIncludeMatches instanceof Boolean && hashedAttribute instanceof Integer && hashedValue instanceof Integer) {
                        assertTrue(provider.avfIsActive);
                        assertEquals(attributeValueFilter.getBoolean("i"), provider.avfShouldIncludeMatches);
                        assertEquals(attributeValueFilter.getInt("a"), provider.avfHashedAttribute);
                        assertEquals(attributeValueFilter.getInt("v"), provider.avfHashedValue);
                    }
                    else {
                        assertFalse(provider.avfIsActive);
                    }
                }
            }
            if (ekConfig.has("hs")) {
    ...
    public void testAttributeValueFiltering() throws Exception {
        ...
        AbstractKit includeTrueProvider, includeFalseProvider;
        {
            includeTrueProvider = new FakeProvider().setKitManager(Mockito.mock(KitManager.class));
            includeFalseProvider = new FakeProvider().setKitManager(Mockito.mock(KitManager.class));
            includeTrueProvider.parseConfig(new JSONObject(ATTRIBUTE_VALUE_FILTERING_INCLUDE_TRUE));
            includeFalseProvider.parseConfig(new JSONObject(ATTRIBUTE_VALUE_FILTERING_INCLUDE_FALSE));
        }

        boolean result;

        result = includeTrueProvider.shouldIncludeFromAttributeValueFiltering(matchingAttributes);
        assertTrue(result);
        result = includeTrueProvider.shouldIncludeFromAttributeValueFiltering(halfMatchingAttributes);
        assertFalse(result);
        result = includeTrueProvider.shouldIncludeFromAttributeValueFiltering(nonMatchingAttributes);
        assertFalse(result);

        result = includeFalseProvider.shouldIncludeFromAttributeValueFiltering(matchingAttributes);
        assertFalse(result);
        result = includeFalseProvider.shouldIncludeFromAttributeValueFiltering(halfMatchingAttributes);
        assertTrue(result);
        result = includeFalseProvider.shouldIncludeFromAttributeValueFiltering(nonMatchingAttributes);
        assertTrue(result);
    }
