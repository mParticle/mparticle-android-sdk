        ...
        for (EmbeddedProvider provider : providers.values()){
            try {
                if (!provider.optedOut() && provider.shouldLogEvent(type, name)) {
                    provider.logEvent(type, name, provider.filterEventAttributes(type.toString(), name, provider.mAttributeFilters, eventAttributes));
                }
            } catch (Exception e) {
        ...
        for (EmbeddedProvider provider : providers.values()){
            try {
                if (!provider.optedOut() && provider.shouldLogScreen(screenName)) {
                    provider.logScreen(screenName, provider.filterEventAttributes("0", screenName, provider.mScreenAttributeFilters, eventAttributes));
                }
            } catch (Exception e) {
