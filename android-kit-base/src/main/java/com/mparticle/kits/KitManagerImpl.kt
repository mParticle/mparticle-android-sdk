...
open class KitManagerImpl( // ================================================================================
    ...
    override fun setLocation(location: Location?) {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call setLocation for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun logNetworkPerformance(
        ...
        for (provider in providers.values) {
            try {
                if (!provider!!.isDisabled()) {
                    val report: List<ReportingMessage?> = provider.logNetworkPerformance(
                        url,
                        startTime,
                        method,
                        length,
                ...
                Logger.warning("Failed to call logNetworkPerformance for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
        ...
        for (provider in providers.values) {
            try {
                if (!provider!!.isDisabled(true)) {
                    val messages: List<ReportingMessage?> = provider.setOptOut(optOutStatus)
                ...
                Logger.warning("Failed to call setOptOut for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
        reloadKits()
    }
    ...
    override fun logEvent(event: BaseEvent) {
        val event: BaseEvent? = mDataplanFilter.transformEventForEvent(event)
        for (provider in providers.values) {
            try {
                val messages: List<ReportingMessage?> = provider!!.logBaseEvent(
                    event!!
                )
                mCoreCallbacks.kitListener.onKitApiCalled(
                    provider.configuration!!.kitId,
                ...
                Logger.warning("Failed to call logMPEvent for kit: " + provider!!.getName() + ": " + e.message)
                mCoreCallbacks.kitListener.onKitApiCalled(
                    provider.configuration!!.kitId,
                    false,
                    event,
    ...
    protected open fun logCommerceEvent(event: CommerceEvent) {
        for (provider in providers.values) {
            try {
                    ...
                    if (filteredEvent != null) {
                        if (provider is CommerceListener) {
                            ...
                            if (projectedEvents != null && projectedEvents.size > 0) {
                                ...
                                val reporting: List<ReportingMessage?> =
                            ...
                            if (events != null) {
                                for (expandedEvent in events) {
                                    ...
                                    forwarded = forwarded || reporting.isNotEmpty()
                                }
                            }
                            if (forwarded) {
                                reportingManager.log(
                ...
                Logger.warning("Failed to call logCommerceEvent for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
        ...
        for (provider in providers.values) {
            if (provider is PushListener) {
                try {
                    ...
                    Logger.warning("Failed to call onPushMessageReceived for kit: " + provider.getName() + ": " + e.message)
                }
            }
        }
        return false
        ...
        for (provider in providers.values) {
            if (provider is PushListener) {
                try {
                    ...
                    Logger.warning("Failed to call onPushRegistration for kit: " + provider.getName() + ": " + e.message)
                }
            }
        }
        return false
    ...
    override fun onUserAttributesReceived(
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call setUserAttributes for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun setUserAttribute(attributeKey: String, attributeValue: String?, mpid: Long) {
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call setUserAttributes/onSetUserAttribute for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun removeUserAttribute(key: String, mpid: Long) {
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call removeUserAttribute/onRemoveUserAttribute for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun incrementUserAttribute(
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onIncrementUserAttribute for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun setUserTag(tag: String, mpid: Long) {
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onSetUserTag for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun setUserIdentity(id: String, identityType: MParticle.IdentityType) {
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call setUserIdentity for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun removeUserIdentity(identityType: MParticle.IdentityType) {
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call removeUserIdentity for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun logout() {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call logout for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
            ...
            try {
                if (provider is KitIntegration.EventListener && !provider.isDisabled() && provider.configuration!!.shouldLogEvent(
                    ...
                    if (projectedEvents == null) {
                        ...
                        for (i in projectedEvents.indices) {
                            val projectedEvent = projectedEvents[i].mpEvent
                            val messages: List<ReportingMessage?> =
    ...
    override fun logBatch(batch: String) {
        for (provider in providers.values) {
            try {
                ...
                Logger.error(
                    jse,
                    "Failed to call logBatch (unable to deserialize Batch) for kit" + provider!!.getName() + ": " + jse.message
                )
            } catch (e: Exception) {
                Logger.warning("Failed to call logBatch for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
        ...
        for (provider in providers.values) {
            try {
                if (provider is KitIntegration.EventListener && !provider.isDisabled()) {
                    val report: List<ReportingMessage?> =
                ...
                Logger.warning("Failed to call leaveBreadcrumb for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun logError(message: String, eventData: Map<String, String?>?) {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call logError for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun logException(
        ...
        for (provider in providers.values) {
            try {
                if (provider is KitIntegration.EventListener && !provider.isDisabled()) {
                    val report: List<ReportingMessage?> =
                        (provider as KitIntegration.EventListener).logException(
                            exception,
                            eventData,
                            message
                        )
                ...
                Logger.warning("Failed to call logException for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
            ...
            try {
                if (provider is KitIntegration.EventListener && !provider.isDisabled() && provider.configuration!!.shouldLogScreen(
                    ...
                    if (projectedEvents == null) {
                        ...
                        val report: List<ReportingMessage?> =
                            (provider as KitIntegration.EventListener).logScreen(
                                eventName,
                                eventInfo
                            )
                        mCoreCallbacks.kitListener.onKitApiCalled(
                ...
                Logger.warning("Failed to call logScreen for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onActivityCreated(activity: Activity, savedInstanceState: Bundle?) {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onActivityCreated for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onActivityStarted(activity: Activity) {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onActivityStarted for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onActivityResumed(activity: Activity) {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onActivityResumed for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onActivityPaused(activity: Activity) {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onResume for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onActivityStopped(activity: Activity) {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onActivityStopped for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onActivitySaveInstanceState(activity: Activity, outState: Bundle?) {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onActivitySaveInstanceState for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onActivityDestroyed(activity: Activity) {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onActivityDestroyed for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onSessionEnd() {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onSessionEnd for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onSessionStart() {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onSessionStart for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onApplicationForeground() {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onApplicationForeground for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onApplicationBackground() {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onApplicationBackground for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
        ...
        if (instance != null) {
            ...
            for (provider in providers.values) {
                try {
                    ...
                    Logger.warning("Failed to update Install Referrer for kit: " + provider!!.getName() + ": " + e.message)
                }
            }
        }
    }
    ...
    override fun onUserIdentified(mParticleUser: MParticleUser, previousUser: MParticleUser?) {
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onUserIdentified for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    ...
    override fun onIdentifyCompleted(
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onIdentifyCompleted for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onLoginCompleted(
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onLoginCompleted for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onLogoutCompleted(
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onLogoutCompleted for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
    ...
    override fun onModifyCompleted(
        ...
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call onModifyCompleted for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
        ...
        for (provider in providers.values) {
            if (provider is KitIntegration.UserAttributeListener && !provider.isDisabled()) {
                try {
                    ...
                    Logger.warning("Failed to call onConsentStateUpdated for kit: " + provider.getName() + ": " + e.message)
                }
            }
        }
    }
    ...
    override fun reset() {
        for (provider in providers.values) {
            try {
                ...
                Logger.warning("Failed to call reset for kit: " + provider!!.getName() + ": " + e.message)
            }
        }
    }
