...
public class CustomMappingTest {
        ...
        for (int i = 0; i < ekConfigs.length(); i++) {
            ...
            if (config.getInt("id") == 56) {
                ...
                for (int j = 0; j < projJsonList.length(); j++) {
                    ...
                    assertEquals(match.getInt("message_type"), customMapping.getMessageType());
                    assertEquals(match.getString("event_match_type"), customMapping.mMatchType);
                    if (customMapping.mMatchType.startsWith(CustomMapping.MATCH_TYPE_HASH)) {
                        assertEquals(match.getInt("event"), customMapping.mEventHash);
                    } else {
                        assertEquals(match.getString("event"), customMapping.mEventName);
                    }
                    if (match.has("attribute_key")) {
                        assertEquals(match.getString("attribute_key"), customMapping.mAttributeKey);
                        assertEquals(match.getString("attribute_value"), customMapping.mAttributeValue);
                    }
    ...
    public void testDefault() throws Exception {
        CustomMapping defaultCustomMapping = new CustomMapping(new JSONObject("{ \"id\":89, \"match\":{ \"message_type\":4, \"event_match_type\":\"\", \"event\":\"\" }, \"behavior\":{ \"append_unmapped_as_is\":true, \"is_default\":true }, \"action\":{ \"projected_event_name\":\"\", \"attribute_maps\":[ ] } }"));
    ...
    public void testDontAppendAsIs() throws Exception {
        CustomMapping customMapping = new CustomMapping(new JSONObject("{ \"id\":89, \"match\":{ \"message_type\":4, \"event_match_type\":\"\", \"event\":\"\" }, \"behavior\":{ \"append_unmapped_as_is\":false, \"is_default\":true }, \"action\":{ \"projected_event_name\":\"\", \"attribute_maps\":[ ] } }"));
        HashMap<String, String> info = new HashMap<String, String>();
        info.put("some key", "some value");
        info.put("another key", "another value");
        MPEvent event = new MPEvent.Builder("whatever", MParticle.EventType.Other).info(info).build();
        ...
        customMapping = new CustomMapping(new JSONObject("{ \"id\":89, \"match\":{ \"message_type\":4, \"event_match_type\":\"\", \"event\":\"\" }, \"behavior\":{ \"append_unmapped_as_is\":true, \"is_default\":true }, \"action\":{ \"projected_event_name\":\"\", \"attribute_maps\":[ ] } }"));
    ...
    public void testMaxParams() throws Exception {
        CustomMapping customMapping = new CustomMapping(new JSONObject("{ \"id\":89, \"match\":{ \"message_type\":4, \"event_match_type\":\"\", \"event\":\"\" }, \"behavior\":{ \"append_unmapped_as_is\":true, \"max_custom_params\":5, \"is_default\":true }, \"action\":{ \"projected_event_name\":\"\", \"attribute_maps\":[ ] } }"));
        HashMap<String, String> info = new HashMap<String, String>();
        ...
        customMapping = new CustomMapping(new JSONObject("{ \"id\":89, \"match\":{ \"message_type\":4, \"event_match_type\":\"\", \"event\":\"\" }, \"behavior\":{ \"append_unmapped_as_is\":true, \"max_custom_params\":5, \"is_default\":true }, \"action\":{ \"projected_event_name\":\"\", \"attribute_maps\":[ { \"projected_attribute_name\":\"\", \"match_type\":\"String\", \"value\":\"key 6\", \"data_type\":\"String\" }] } }"));
        ...
        customMapping = new CustomMapping(new JSONObject("{ \"id\":89, \"match\":{ \"message_type\":4, \"event_match_type\":\"\", \"event\":\"\" }, \"behavior\":{ \"append_unmapped_as_is\":true, \"is_default\":true }, \"action\":{ \"projected_event_name\":\"\", \"attribute_maps\":[ { \"projected_attribute_name\":\"\", \"match_type\":\"String\", \"value\":\"key 6\", \"data_type\":\"String\" }] } }"));
        result = customMapping.project(new EventWrapper.MPEventWrapper(event)).get(0);
        newEvent = result.getMPEvent();
        assertTrue(newEvent.getInfo().containsKey("key 5"));
        assertTrue(newEvent.getInfo().containsKey("key 6")); //note that the json also doesn't specify a key name - so we just use the original
    ...
    public void testMultiHashProjections() throws Exception {
        ...
        eventList = CustomMapping.projectEvents(builder.build(), kitConfiguration.getCustomMappingList(), kitConfiguration.getDefaultEventProjection());
